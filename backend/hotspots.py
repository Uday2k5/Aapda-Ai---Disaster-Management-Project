...
WILDFIRE_CITIES = [
    {"name": "Dehradun", "country": "India", "latitude": 30.3165, "longitude": 78.0322},
...
def get_hotspots() -> dict[str, list[dict[str, object]]]:
    ...
    wildfire = []
    for city in WILDFIRE_CITIES:
        result = predict_wildfire(city["latitude"], city["longitude"])
        wildfire.append(_city_payload(city, "wildfire", result))

    flood.sort(key=lambda item: item["risk_score"], reverse=True)
    earthquake.sort(key=lambda item: item["risk_score"], reverse=True)
    wildfire.sort(key=lambda item: item["risk_score"], reverse=True)
    return {"flood": flood, "earthquake": earthquake, "wildfire": wildfire}
